...
abstract class BaseMvpFragment<V : IBaseFragmentMvpView, P : IBaseFragmentMvpPresenter<V>> : Fragment(), IBaseFragmentMvpView {
    @Inject
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // inject appComponent
        appContext.appComponent.inject(this as BaseMvpFragment<IBaseFragmentMvpView, IBaseFragmentMvpPresenter<IBaseFragmentMvpView>>)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // update toolbar
        OptionalUtility.with(mActivity.appSupportActionBar)
                .doIfPresent { actionBar ->
                    subscribeWith {
                        Observable.just("")
                                .delayEach(Constant.TIME_DELAY_INIT_SCREEN, TimeUnit.MILLISECONDS)
                                .subscribeOn(Schedulers.io())
                                .observeOn(AndroidSchedulers.mainThread())
                                .subscribe { initToolbar(actionBar) }
                    }
                }

        // update title
        StringUtility.with(titleString)
                .doIfEmpty { mActivity.setToolbarTitle(titleId) }
                .doIfPresent { mActivity.setToolbarTitle(titleString) }
    }
    ...
    override fun initToolbar(supportActionBar: ActionBar) {
        // elevation
        mActivity.appBarLayout?.elevation = mActivity.getResources().getDimension(R.dimen.elevation_header)
    ...
    override fun showLoadingView(isLoading: Boolean) {
        OptionalUtility.with(mActivity).doIfPresent { it.showLoadingView(isLoading) }
    }

    /**
     * @param isChildFragment Used to mark a fragment used in the viewpager
     * It does not change title, icon.. of parent fragment
     */
    fun setChildFragment(isChildFragment: Boolean): BaseMvpFragment<V, P> {
