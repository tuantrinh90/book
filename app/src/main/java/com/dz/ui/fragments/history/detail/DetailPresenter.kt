...
class DetailPresenter(appComponent: AppComponent, mContext: Context) : BaseFragmentMvpPresenter<IDetailView>(appComponent), IDetailPresenter {
    ...
    override fun setBookDetail() {
        getView {
            Observable.fromCallable { dataModule.getBookDetail() }
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribeWith(object : DisposableObserver<List<BookDetail>>() {
                        override fun onNext(t: List<BookDetail>) {
                           it.getBookDetail(t)
                        }
